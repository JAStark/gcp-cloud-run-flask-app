resource "google_cloudbuild_trigger" "prod-flask-app-filename-trigger" {
  ...
  build {
    images = ["europe-west1-docker.pkg.dev/$PROJECT_ID/prod-gcp-cloud-run-flask-app-test/flask-endpoint-image:tag_1"]
    step {
      name        = "gcr.io/cloud-builders/docker"
      args        = ["build", "t", "europe-west1-docker.pkg.dev/$PROJECT_ID/prod-gcp-cloud-run-flask-app-test/flask-endpoint-image:tag_1", "./flask_app_cloud_run/Dockerfile"]
...
resource "google_cloud_run_service" "prod-flask-app-cloud-run" {
  ...
  template {
    spec {
      containers {
        image = "europe-west1-docker.pkg.dev/$PROJECT_ID/prod-gcp-cloud-run-flask-app-example/flask-endpoint-image:tag_1"
      }
    }
  }
